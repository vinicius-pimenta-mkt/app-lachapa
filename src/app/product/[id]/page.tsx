...
export default function ProductDetail({ params }: ProductDetailProps) {
  ...
  const toggleAdditional = (additional: Additional) => {
    const existingIndex = selectedAdditionals.findIndex(a => a.id === additional.id);
    ...
    if (existingIndex >= 0) {
      ...
      setSelectedAdditionals([...selectedAdditionals, { ...additional, quantity: 1 }]);
    }
  };
  
  const handleAdditionalQuantityChange = (additionalId: string, change: number) => {
    setSelectedAdditionals(prevAdditionals => 
      prevAdditionals.map(a => {
        if (a.id === additionalId) {
  ...
  const calculateTotalPrice = () => {
    ...
    selectedAdditionals.forEach(additional => {
      total += additional.price * additional.quantity * quantity;
    });
  ...
  const addToCart = () => {
    // Obter o carrinho atual do localStorage ou inicializar um novo
    const currentCart = localStorage.getItem('cart') 
      ? JSON.parse(localStorage.getItem('cart') || '[]') 
      : [];
    
    // Criar o item para adicionar ao carrinho
    const cartItem = {
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {additionals.map(additional => {
                  ...
                  const selectedAdditional = selectedAdditionals.find(a => a.id === additional.id);
                      ...
                      {isSelected && (
                        <div className="additional-quantity-control">
                          <button 
                            ...
                            disabled={selectedAdditional?.quantity === 1}
                          >
                            -
                          </button>
                          <span className="additional-quantity-display">
                            {selectedAdditional?.quantity || 1}
                          </span>
                          <button 
                            onClick={() => handleAdditionalQuantityChange(additional.id, 1)}
                            className="additional-quantity-button"
